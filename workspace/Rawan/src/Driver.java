import java.util.Random;
	...
	public class Driver 
	{
		public static void main(String[] args)
		{
			...
			Driver.mergeSort(arrayOfNumbers, 0, arrayOfNumbers.length -1);
			Driver.printIntArray(arrayOfNumbers);
		...
		static void mergeSort(int [] ar, int begin, int end)
		{
			...
			// if data is NOT sorted (ie not a one-list)
			Driver.mergeSort(ar, begin, end);
			 if (begin != end){
			// call merge sort on right half
			
			int rightB = ((end-begin)/2 +begin);
			int rightE = end;
			
				// call merge sort on left half
				
				  int leftB = begin;
					int leftE = ((end-begin)/2 + begin);
				
					// do the merge step
					// if data is sort - just return	
					
						
				int[] a = new int[rightE -leftB + 1];
				int Left = leftB;
				int Right = rightB;
